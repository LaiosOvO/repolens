...
Repo Teacher 是一个本地代码仓库教学报告生成器。它先建立文件、符号与依赖图，再由模型把源码证据归纳成“产品主轴 → 业务功能 → 底层机制 → 难点与取舍 → 工程结构 → 源码证据”，最终为每个仓库生成一个主要给人阅读的 `index.html`。
...
2. **产品主轴**：最多四条，先建立项目的产品心智模型。
3. **业务功能**：用户能完成什么；框架项目则是开发者能构建或控制什么。
4. **实现机制**：跨哪些前端、后端、Worker、媒体、状态和协议模块完成。
5. **真正难点**：不变量、失败方式、当前取舍和生产边界。
6. **工程结构**：monorepo/单包/多服务，DDD/分层/模块化等组织判断。
