...
2. **用户能直接体验的业务功能**：按用户目标逐项用 1–3 句自然语言说明“在哪里做什么 → 系统完成什么 → 看见什么结果”，并标当前支持边界；不是页面/接口名清单。
3. **核心功能有哪些**：不限数量，每项先说明它支撑哪个业务结果，再用一句当前证据推导的自然语言说明实现本质和最关键的运行判定；不得只放名词卡片。这里建立业务功能到核心机制的映射。
4. **核心架构**：解释调用方/前端、后端/运行时、状态、Worker 与外部系统如何共同完成最核心的一次运行。仓库没有前端或服务端时直接写“不存在”。
5. **技术栈与依赖实现**：紧接核心架构，按前端、后端/运行时、数据、通信/异步、AI/Agent、构建部署列真实依赖；每项说明实际入口、项目职责、交接协议和 `runtime/direct/transitive/dev-only/declared-only` 边界，不做技术 logo 墙。
6. 核心概念与一次真实用户旅程。
7. 各核心功能；每章就地讲完整底层运行。
8. 工程目录地图、前后端/进程/Worker/数据/部署。
9. 覆盖账本和证据边界。
