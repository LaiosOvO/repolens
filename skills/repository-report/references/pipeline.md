...
功能识别仍由产品声明和全量 surface 账本决定。对每个 surface，将其入口/结果证据映射到 CodeGraph 节点，再沿确定关系向下游和上游扩展，得到调用、读写、发布/消费、Worker 与外部适配关系。图上不可解的跳转必须回到源码核验并标为缺口，不得用名称相似性接边。
...
再从十三项证据中生成**逐跳因果账本**。不能只写符号箭头；每一行必须让下一行能够接住上一行的具体输出：

| 时点/生命周期 | 本跳输入对象与关键字段 | 控制者读取什么 | 判定/转换规则 | 本跳输出对象或已提交状态 | 下一消费者 | 失败/取消怎样停 | 源码 |
|---|---|---|---|---|---|---|---|

账本行数由真实边界决定，不设固定值。只要跨越公开入口、模型/外部服务、validator/compiler、持久化、队列/调度器、Worker/工具、事件/结果消费者中的任何边界，就至少要有一行；不存在的边界不虚构。若定义/配置与实际运行由不同触发器完成，分别形成“定义生命周期”和“运行生命周期”，并用“被保存的哪个对象/版本/ID 在运行时被谁加载”明确连接。随后从账本提炼自然语言因果主链；正文、规则表和图共享事实，但只有正文完整解释。
...
- 是否先说明了功能的用户、场景、输入、系统责任、可见结果和明确非目标，而不是一上来解释类型名？
- 若功能有创建/配置/编译与执行两个时点，是否分别讲清并说明保存对象如何成为运行输入？
- 因果账本相邻两行是否真正满足“上一行输出 = 下一行输入”；是否有任何中间对象突然出现而没有创建者？
